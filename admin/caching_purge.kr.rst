...
-  ``<Purge2Expire> (기본: NONE)``
   ...
   예를 들어 전체(/**)를 `Purge`_ 하는 경우 의도하지 않게 많은 컨텐츠가 
...
-  ``<RootPurgeExpire> (기본: ON)``
   ...
   전체(/**)에 대한 의도하지 않은 `Purge`_ / `Expire`_ 는 과도한 원본서버 
   부하를 발생시킬 수 있다. 
   이 설정을 통하여 전체(/**)에 대한 `Purge`_ / `Expire`_ 를 차단할 수 있다. 
...
대상 지정은 URL, 패턴, 디렉토리 3가지로 표현한다. ::

   example.com/logo.jpg
   example.com/img/*.jpg
   example.com/img/
   
명확한 URL 외에 패턴(*.jpg)이나 디렉토리(/img/)도 무효화가 가능하다.
...
.. note::
   ...
   예를 들어 사용자가 example.com에 접근하면 루트 디렉토리(/)에 대한 HTTP 요청을 보낸다. ::
   
      GET / HTTP/1.1
      Host: example.com
   
   이에 대해 웹서버는 관리자가 설정한 특정 페이지(아마도 index.html 또는 index.htm)로 응답한다.
   분명 웹 서비스 구성에서 루트 디렉토리(/)는 디렉토리가 아닌 페이지로 동작한다.
   
   하지만 Cache서버는 루트 디렉토리(/)에 접근했더니 200 OK 페이지가 왔다고 이해한다.
   심지어 원본서버가 어떤 페이지로 응답했는지 알 수 없다.
   
   따라서 이에 대한 명확한 예외적 표현이 필요하며 `Purge`_ 등의 대상으로 지정할 때 다음과 같이 규격화 하였다. ::
   
      example.com/img/          // example.com 가상호스트의 /img폴더와 그 하위
      example.com/              // example.com 가상호스트의 루트 페이지(/)
      example.com/**            // example.com 가상호스트의 전체
