...
-  ``<Purge2Expire> (기본: NONE)``
   ...
   예를 들어 특정 디렉토리 또는 전체(/**)를 `Purge`_ 하는 경우 의도하지 않게 많은 컨텐츠가 
...
패턴이나 디렉토리 표현은 서비스에서 배제된 상태에서 관리용도로 사용하기 위함이다.
...
.. note::
   ...
   심지어 원본서버가 어떤 페이지로 응답했는지 알지 못한다. 
   따라서 기본 페이지(/)에 대한 명확한 예외적 표현(디렉토리가 아니다)이 필요하다. 
   정리하면 아래와 같다. ::
