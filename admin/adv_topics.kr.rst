...
특히 인덱싱(요청된 URL을 빠르게 찾는 것)에 대해서는 반드시 이해하길 바란다.
...
해당 콘텐츠가 접근될 때마다 매번 디스크에서 읽어서 전송하면 당연히 성능이 저하된다.
...
서비스 중단없이 디스크를 교체한다. 
파라미터는 반드시 ``<디스크>`` 설정과 같아야 한다. ::

   http://127.0.0.1:10040/command/unmount?디스크=...
   http://127.0.0.1:10040/command/umount?디스크=...
