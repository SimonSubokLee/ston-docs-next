...
::
   ...
   <Compression Method="gzip" Level="9" SourceSize="2-2048">OFF</Compression>
...
-  ``<Compression>``
   ...
   -  ``ON`` 압축 기능을 사용한다. 다음 속성을 지원한다.
      ...
      -  ``Method (기본: gzip)`` 압축 방식을 지정한다. 현재는 gzip만 지원된다.
      -  ``Level (기본: 6)`` 압축 단계를 지정한다. 이 값은 ``Method`` 에 따라 달라진다. gzip은 1~9까지 지정이 가능하다. 
         숫자가 적을수록 빠르지만 압축률이 나쁘고, 클수록 느리지만 압축률이 좋다.
...
압축 대상은 /svc/{vhost}/compression.txt 에 지정한다. 정의된 순서대로 적용된다. ::
   ...
   *.htm, gzip           // *.htm 조건에 대해 deflate로 압축한다. (기본 Level)
   *.xml, , 9            // *.xml 조건에 대해 Level 9로 압축한다.*. (기본 Method)
   *.js, gzip, 5         // *.js 조건에 대해 gzip (Level=5)로 압축한다.*.
